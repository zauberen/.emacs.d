...
(use-package lsp-java
  ...
  :hook (java-ts-mode . (lambda ()
                          ...
                          ;TODO: Should update my/dont-use-lsp-on-windows to just do this for everything (except maybe js?)
                          (when (length> (-filter (lambda (name) (string= name "*jdtls*"))
                                                  (mapcar 'buffer-name (buffer-list))) 0)
                            (lsp))))
