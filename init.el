...
(use-package substitute
  ...
  :config
  (defun substitute-all-targets-in-buffer (target sub &optional fixed-case)
    "Replace data even if it is outside of the narrowed context."
    (interactive
     (let ((target (substitute--determine-target)))
       (list target
             (substitute--prompt target nil)
             current-prefix-arg)))
    (let ((substitute-ignore-narrowing t))
      (substitute--operate target sub nil fixed-case)))
